...
- `ansible` (+ коллекции: `make deps`), `k0sctl`, `make`;
  для `make lint` — `ansible-lint`, `yamllint`.
- **`pip install kubernetes`** — работа с кластером идёт модулями
  `kubernetes.core` (`k8s`, `k8s_info`), а им нужен этот python-пакет на
  хосте-операторе. Без него `make storage` и кластерная часть `make check`
  упадут с «Failed to import the required Python library».
  Отдельный `kubectl` для самого провижининга не нужен.
...
2. Проверки перед прогоном:
   ...
   когда узлы перенастроены. Перечисляет сразу все пробелы, а не по одному
   за прогон.
