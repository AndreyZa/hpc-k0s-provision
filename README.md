...
**Что именно резервируется тремя контроллерами.** HA здесь относится к
**хранилищу состояния**: кворум etcd переживёт потерю одного контроллера.
Точка входа в API — НЕ резервируется: `k0sctl kubeconfig` записывает в
`./kubeconfig` адрес первого контроллера, и весь инструментарий (харнесс,
statusserver, make-таргеты, энергоскрипты) ходит по нему. Падение именно
этого узла останавливает серию, хотя кластер жив. Отдельного VIP/балансировщика
IaC не поднимает — сознательно, чтобы не тянуть keepalived на стенд ради
одного адреса. Если контроллер потерян посреди кампании: переписать `server:`
в kubeconfig на живой контроллер (сертификат API это допускает — k0s кладёт в
SAN адреса всех контроллеров) и перезапустить прогон. Если такой простой
неприемлем — поднять VIP и указать его в `spec.api.externalAddress`.
...
Ставить руками нужно немного — остальное доставляет `make deps` (и `preflight`
внутри `provision` вызывает его сам, если чего-то нет):

- `ansible`, `make`, `git`, `curl`; для `make lint` — `ansible-lint`, `yamllint`.
...
- bench-репозиторий рядом (`BENCH_REPO`, по умолчанию `../sensitivityscore-hpc-bench`).
- `docker` **и compose v2 как плагин** (`docker compose version` должен
  отвечать) — на нём поднимается статус-страница серии. В Ubuntu плагин лежит
  в отдельном пакете `docker-compose-v2` и с самим docker'ом не приезжает; без
  него лаунчер серии падает на непрозрачном `unknown shorthand flag: 'f' in -f`.
  Для провижининга docker не нужен — только для прогонов с этого хоста.

`make deps` доставляет в сам репозиторий, не в систему:

- ansible-коллекции (`requirements.yml`);
- `k0sctl` фиксированной версии → `./bin` (Makefile кладёт его в начало `PATH`,
  так что версия стенда воспроизводима и не зависит от того, что стоит в системе);
- python-пакет `kubernetes` → `./.venv`. Он нужен модулям `kubernetes.core`
  (`k8s`, `k8s_info`), которыми идёт вся работа с кластером — отдельный
  `kubectl` для провижининга не требуется. Ставим в venv, а не в систему,
  потому что системный python на macOS и свежих Debian/Ubuntu помечен
  externally-managed (PEP 668) и `pip install` туда откажет. Какой
  интерпретатор возьмут модули, решает `inventory/host_vars/localhost.yml`:
  эта `.venv`, если она есть, иначе прежний python (значит пакет уже стоит
  глобально — тоже рабочий вариант).

Убрать поставленное: `make deps-clean`.
...
2. Проверки перед прогоном:
   ...
   ```bash
   make preflight # инструментарий оператора: ansible+коллекции, python-kubernetes, k0sctl, bench-репа
   ...
   `preflight` — первый шаг `provision`, отдельно запускать не обязательно.
   Он существует потому, что нехватка `k0sctl` или `python-kubernetes`
   всплыла бы только на шагах `cluster`/`storage`, то есть уже после `prep`,
   когда узлы перенастроены. Что может — доставляет сам (`make deps`), что
   не может — перечисляет сразу всё, а не по одному за прогон: сам `ansible`
   (им же ставится остальное), инвентарь и bench-репу. `PREFLIGHT_INSTALL=0` —
   только проверить, ничего не меняя (CI, чужая машина).
...
3. Полный цикл:
   ...
   Это: `preflight` (окружение) → `prep` (ОС) → `cluster` (k0s, пишет
   `./kubeconfig`) → `testbed` (роли + Redis/scheduler/agent через bench-репу)
   → `storage` (StorageClass по умолчанию) → `clickhouse` (in-cluster приёмник
   результатов) → `monitoring` (Prometheus + Grafana) → `check`.

   Порядок шагов не произвольный: `clickhouse` и `monitoring` прибиты к
   ss-system и потому идут после `testbed` (он вешает метку роли), а
   `clickhouse` — ещё и после `storage`, иначе его PVC повиснет в Pending.

Гранулярно — те же шаги по отдельности: `make prep`, `make cluster`,
`make testbed`, `make storage`, `make clickhouse`, `make monitoring`,
`make check`. `make help` — список целей.
